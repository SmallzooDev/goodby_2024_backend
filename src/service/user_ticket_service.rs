...
impl UserTicketService {
    ...
    pub async fn create_tickets_for_users(
        ...
        let mut tx = self.db_conn.as_ref().get_pool().begin().await.map_err(|e| {
            ApiError::DbError(DbError::SomethingWentWrong(e.to_string()))
        })?;
        ...
        tx.commit().await.map_err(|e| ApiError::DbError(DbError::SomethingWentWrong(e.to_string())))?;
